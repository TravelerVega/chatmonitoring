...
function Get_Messagedata(s)
    ...
    return (os.time() - current_messages[s]), config.max_delay + (current_messages[s] - os.time())
end
...
function check_cooldown(author)
    local Msg_time, time_left = Get_Messagedata(author)
    if Msg_time <= config.max_delay then
    ...
    for i=1,10 do 
        if user[i] then
            ...
            if thing then
                ...
                print(time_left)
                if time_left ~= 0 and time_left >= 0 then
                    ...
                    elseif min > 0 then
                        ...
                        TriggerClientEvent("chatMessage", author, "^1You are muted! Try again in "..sec.." second(s)")
                        return true
                    end
                end
            end
...
function checkIfCanSendMessage(Source)
    if not check_muted(Source) and not check_cooldown(Source) then
        return true
    end
    return false
end
...
RegisterCommand("mute", function(s,a,r)
    ...
    if true then
        if Get_UserMute(license) ~= 0 and Get_UserMute(license) >= 0  then
    ...
    for k,v in pairs(muted_players) do
        ...
        if time_secs ~= 0 then
            ...
            elseif min > 0 then
                ...
                TriggerClientEvent("chatMessage", s, "^1User: "..GetPlayerName(k).." : "..sec.." second(s)")
            end
        end
    end
end, false)

if not config.other_chat then
    AddEventHandler('chatMessage', function(author, color, text)
        if check_cooldown(author) then
            CancelEvent()
        end
        if check_muted(author) then
            CancelEvent()
        end
    end)
end
